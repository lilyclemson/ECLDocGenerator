...
tag_template = latex_jinja_env.get_template(
    joinpath(TEX_TEMPLATE_DIR, 'taglets.tpl.tex'))
...
def render_param(tag_param):
    if len(tag_param.tuples['tuples']) == 0:
        return ''
    tex_tuples = []
    for t in tag_param.tuples['tuples']:
        tex_tuples.append((t[0], construct_type(t[2]), t[1]))

    render = tag_template.render(render_name='threetag', args=[
                                 'Parameter', tex_tuples])
    return render


def render_field(tag_field):
    if len(tag_field.tuples['tuples']) == 0:
        return ''
    html_tuples = []
...
def render_return(tag_return):
    if len(tag_return.docstrings) == 0 and tag_return.return_type is None:
        return ''
    return_tuple = [
        (construct_type(tag_return.return_type), tag_return.return_desc)]
...
def render_parent(tag_parent):
    if len(tag_parent.tuples['tuples']) == 0:
        return ''
    render = tag_template.render(render_name='linktag', args=[
                                 'Parent', tag_parent.tuples['tuples']])
    return render
...
def render_inherit(tag_inherit):
    if tag_inherit == 'local':
        return ''
    render = tag_template.render(render_name='onetag', args=[
                                 tag_inherit, [('', )]])
    return render
